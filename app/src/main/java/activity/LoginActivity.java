...
public class LoginActivity extends AccountAuthenticatorActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mEditUserEmail = (EditText) findViewById(R.id.email);
	...
	public void login(View view) {
		if(kinveyClient.user().isUserLoggedIn()){
			...
			if (TextUtils.isEmpty(mUserEmail) || TextUtils.isEmpty(mPassword)) {
				...
				System.out.println("before user login function");
//			showProgress();
				userLogin();
			}
		}
	...
	public void registerAccount(View v) {
		Intent intent = new Intent(this, activity.RegisterNewAccountActivity.class);
	...
	private void finishConfirmCredentials(boolean result) {
		final Account account = new Account(mUserEmail, activity.UserLogin.ACCOUNT_TYPE);
		mAccountManager.setPassword(account, mPassword);
		mAccountManager.setUserData(account, activity.UserLogin.LOGIN_TYPE_KEY, PARAM_LOGIN_TYPE_KINVEY);
	...
	private void finishLogin(String authToken) {
		...
		if (mRequestNewAccount) {
			...
			userData.putString(activity.UserLogin.LOGIN_TYPE_KEY, PARAM_LOGIN_TYPE_KINVEY);
		...
		intent.putExtra(AccountManager.KEY_ACCOUNT_TYPE, activity.UserLogin.ACCOUNT_TYPE);
		...
		if(kinveyClient.user().isUserLoggedIn()){
			...
			kinveyClient.user().login(mEditUserEmail.getText().toString(), mEditPassword.getText().toString(), new KinveyUserCallback() {
				...
				public void onFailure(Throwable t) {
					System.out.println("login Failure callback");
